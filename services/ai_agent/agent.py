...
class AIAgent:
    ...
    async def _get_cached_price(self, product_name: str, store_name: str) -> Optional[Dict]:
    ...
    async def _cache_price(self, product_name: str, store_name: str, price_data: Dict):
        ...
        try:
            ...
            cache_key = self._get_cache_key(product_name, store_name)
            r.setex(cache_key, self.cache_ttl, json.dumps(price_data, default=str))
    ...
    async def scrape_store_real(
        ...
        try:
            ...
            if not price_data:
                print(f"[REAL SCRAPER] Could not scrape price from {store_name}")
                return None
    ...
    async def find_best_price(
        ...
        print(f"[AI AGENT] Mode: {'REAL SCRAPER' if use_real_scraper else 'MOCK DATA'}")
        ...
        if failed_stores and len(results) < max_stores:
            ...
            mock_supplement = [m for m in mock_results if m["store_name"] in failed_stores]
        ...
        for r in results:
            ...
            print(f"[AI AGENT]   {source_emoji} {r['store_name']}: {r['price']} PLN")

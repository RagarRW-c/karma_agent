...
@shared_task(name="services.scraper.tasks.scrape_shop_product", bind=True, acks_late=True, max_retries=3, default_retry_delay=60)
...
def scrape_all():
    db = SessionLocal()
    try:
        ids = [sp.id for sp in db.query(models.ShopProduct).order_by(models.ShopProduct.id).all()]
    finally:
        db.close()
